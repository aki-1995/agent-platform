...
/** Size at which the codex log database is dropped, in bytes. */
const CODEX_LOG_DB_MAX_BYTES = Number(process.env.CODEX_LOG_DB_MAX_BYTES) || 512 * 1024 * 1024
 ...
 * `sessions/`, which this leaves alone. Runs at boot, before any codex process
 * is spawned, so nothing holds the files open.
 */
function pruneCodexLogs(codexDir: string): void {
  let names: string[]
  try {
